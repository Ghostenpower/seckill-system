            ...
            <el-col :span="6">
                <el-space direction="vertical" alignment="center" >
                    <el-text :type="flashTime.isEnding.value == true ? 'danger' : 'info'" size="large">
                        {{ flashTime.msg.value }}{{ flashTime.time.value }}
                    </el-text>
                    <el-button type="primary" size="large" @click="setFlashSale()">立即秒杀</el-button>
                    <el-text>
                        <el-tag type="danger" size="small">库存：{{ flashSaleView.total_stock-flashSaleView.sold_count }}</el-tag>
                    </el-text>
                </el-space>
            </el-col>
        </el-row>
